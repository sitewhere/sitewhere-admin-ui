...
export default class TenantEngineEditor extends Vue implements WithRoute {
  ...
  async refresh() {
    if (this.identifier && this.tenantToken) {
      this.loaded = false;
      const response: AxiosResponse<ITenantEngineConfiguration> = await getTenantEngineConfiguration(
        this.$store,
        this.identifier,
        this.tenantToken
      );
  ...
  async loadScriptCategories() {
    if (this.identifier) {
      const response: AxiosResponse<IScriptCategory[]> = await listScriptCategories(
        this.$store,
        this.identifier
      );
