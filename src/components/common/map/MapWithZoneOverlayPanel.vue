...
export default class MapWithZoneOverlayPanel extends Vue {
  ...
  async loadZoneLayers() {
    ...
    if (map) {
      ...
      const criteria: IZoneSearchCriteria = {
        areaToken: this.areaToken
      };
      const format: IZoneResponseFormat = {};
      const response: AxiosResponse<IZoneSearchResults> = await listZones(
        this.$store,
        criteria,
        format
      );
  ...
  addZonesToFeatureGroup(map: LeafletMap, zones: IZone[]) {
    ...
    zones.forEach(zone => {
      if (this.ignoreZoneTokens.indexOf(zone.token) === -1) {
        const polygon = this.createPolygonForZone(zone);
        if (this.zonesGroup) {
    ...
    if (this.zoomToZones) {
      const zoneBounds: LatLngBounds = this.zonesGroup.getBounds();
  ...
  createPolygonForZone(zone: IZone) {
    const latLngs = swToLeafletBounds(zone.bounds);
