...
export default {
  ...
  methods: {
        ...
        if (configNode && configNode.children) {
          for (let j = 0; j < configNode.children.length; j++) {
            ...
            if (modelNotFound.indexOf(childConfig.name) === -1) {
              let childModel = this.findModelNodeByName(modelNode, childConfig.name)
              if (!childModel) {
      ...
      for (let i = 0; i < childRoles.length; i++) {
        ...
        if (childrenWithRole.length === 0) {
          ...
          for (let j = 0; j < childrenWithRole.length; j++) {
            ...
            let element = this.buildChild(childModel, childConfig, childRoleName,
    ...
    /** Build child entry */
    buildChild: function (childModel, childConfig, childRoleName, childRole,
      childrenWithRole) {
      let child = {}
      ...
      if (childModel.indexAttribute) {
        ...
        child.resolvedIndexAttribute =
          this.resolveIndexAttribute(childModel, childConfig)
        if (childrenWithRole.length === 0) {
