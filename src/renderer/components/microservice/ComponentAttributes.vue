<template>
  <div>
    <v-form v-if="groups && groups.length > 0" v-model="formValid" ref="form"
      lazy-validation>
      <v-tabs v-model="active">
        <v-tabs-bar dark color="primary">
          <v-tabs-item v-for="group in groups" :key="group.uid"
            :href="'#' + group.uid">
            {{ group.name }}
          </v-tabs-item>
        </v-tabs-bar>
        <v-tabs-items>
          <attribute-group-panel v-for="group in groups"
            :attributeGroup="group" :readOnly="readOnly"
            :identifier="identifier" :tenantToken="tenantToken" 
            :key="group.uid" @attributeUpdated="onAttributeUpdated">
...
export default {
  ...
  props: ['context', 'groups', 'readOnly', 'dirty', 'identifier', 'tenantToken'],
