<template>
  <div>
    <v-layout row wrap v-if="elements">
      <v-flex xs12>
        <no-results-panel v-if="elements.length === 0" text="No Elements Found for Group"></no-results-panel>
        <v-data-table
          ...
          :items="elements"
          :hide-actions="true"
          no-data-text="No Elements Found for Group"
        >
          <template slot="items" slot-scope="props">
            ...
            >
              <v-icon class="grey--text text--darken-2 type-icon">fa-microchip</v-icon>
              {{ props.item.device.token }}
            </td>
            <td v-else width="40%" :title="props.item.groupId" :class="elementClassFor(props.item)">
              <v-icon class="grey--text text--darken-2 type-icon">view_module</v-icon>
              {{ props.item.groupId }}
            </td>
    ...
    <device-group-element-delete-dialog ref="delete" :groupToken="token" @elementDeleted="refresh"></device-group-element-delete-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    refresh: function() {
      ...
      listDeviceGroupElements(this.$store, this.token, true, paging)
