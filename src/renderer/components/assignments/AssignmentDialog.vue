    ...
    <base-dialog :title="title" :width="width" :visible="dialogVisible"
      ...
      <v-stepper v-model="step">
        <v-stepper-header>
          ...
          <v-divider></v-divider>
          <v-stepper-step step="2" :complete="secondPageComplete">
            Area Assignment
          </v-stepper-step>
          <v-divider></v-divider>
          <v-stepper-step step="3" :complete="thirdPageComplete">
            Asset Assignment
          </v-stepper-step>
          <v-divider></v-divider>
        ...
        <v-stepper-content step="1">
          <v-card flat>
            <v-card-text>
              <v-checkbox label="Assign device to a customer?"
                v-model="assnAssociateCustomer" light>
              </v-checkbox>
              <customer-chooser v-if="assnAssociateCustomer"
          ...
          <v-card-actions>
            ...
            <v-btn :disabled="!firstPageComplete" flat
        ...
        <v-stepper-content step="3">
          ...
          <v-card-actions>
            ...
            <v-spacer></v-spacer>
            <v-btn flat @click="onCancelClicked">{{ cancelLabel }}</v-btn>
            <v-btn :disabled="!firstPageComplete || !secondPageComplete || !thirdPageComplete"
              flat @click="onCreateClicked">
              {{ createLabel }}
            </v-btn>
            <v-btn :disabled="!firstPageComplete || !secondPageComplete || !thirdPageComplete"
              flat @click="step = 4">
  ...
  computed: {
    ...
    firstPageComplete: function () {
      return this.$data.assnAssociateCustomer
        ? (this.$data.assnCustomerToken != null) : true
    },
