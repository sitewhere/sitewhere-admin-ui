  ...
  <v-card v-if="deviceUnit != null" class="ma-3 elevation-0">
    <v-card-text class="pa-0">
      <v-toolbar v-if="deviceUnit.path == null" class="blue darken-2 white--text" dark>
        ...
        <v-spacer></v-spacer>
        <device-unit-create-dialog :deviceUnit="deviceUnit" @deviceUnitAdded="onDeviceUnitAdded"></device-unit-create-dialog>
      </v-toolbar>
      ...
      <v-card>
        ...
        >
          <device-unit-panel
            ...
            @childUnitDeleted="onChildUnitDeleted"
          ></device-unit-panel>
        </v-card-media>
      </v-card>
    </v-card-text>
...
export default {
  ...
  methods: {
    ...
    onChildUnitDeleted: function(unit) {
      this.deviceUnit.deviceUnits = lodash.reject(this.deviceUnit.deviceUnits, {
        path: unit.path
      });
      this.$emit("contentUpdated");
    },
    ...
    onDeviceSlotDeleted: function(slot) {
      this.deviceUnit.deviceSlots = lodash.reject(this.deviceUnit.deviceSlots, {
        path: slot.path
      });
      this.$emit("contentUpdated");
    },
